...
data['Dependent_count'] = st.selectbox("???", ['Select a dependent_count please','0','1','2','3','4'])
...
data['Education_Level'] = st.selectbox("What's the customer's education level ?", ["Select the customer's education level",'Uneducated','College','High School','Graduate','Post-Graduate','Doctorate','Unknown'])

data['Income_Category'] = st.selectbox("How much does the customer earn a month ?", ['Select an income category please','Less than $40K','Between $40K and $60K','Between $60K and $80K','Between $80K and $120K','Higher than $120K','Unknown'])
...
data['Months_on_book'] = st.slider("How long did they subscribe for ?", 12, 60, key="months_on_book'")

data['Total_Relationship_Count'] = st.select_slider("???", [1,2,3,4,5,6], key="total_relationship_count")

data['Months_Inactive_12_mon'] = st.select_slider("???", [0,1,2,3,4,5,6], key="months_inactive_12_mon")

data['Contacts_Count_12_mon'] = st.select_slider("???", [0,1,2,3,4,5,6], key="contacts_count_12_mon")

data['Credit_Limit'] = st.slider("What's the customer's credit's limit ?", 1000, 40000, key="credit_limit")

data['Total_Revolving_Bal'] = st.number_input('????', min_value=0, max_value=100, key="total_revolving_bal")

data['Total_Amt_Chng_Q4_Q1'] = st.number_input('????', min_value=0, max_value=100, key="total_amt_chng_Q4_Q1")

data['Total_Trans_Amt'] = st.number_input('????', min_value=0, max_value=100, key="total_trans_amt")

data['Total_Trans_Ct'] = st.number_input('????', min_value=0, max_value=100, key="total_trans_ct")

data['Total_Ct_Chng_Q4_Q1'] = st.number_input('????', min_value=0, max_value=100, key="total_ct_chng_Q4_Q1'")

data['Avg_Utilization_Ratio'] = st.number_input('????', min_value=0, max_value=100, key="avg_utilization_ratio")
...
components.html(tableau_dashboard, width=1580, height=927) #Works but a big space on the left
